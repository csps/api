...
export function getTutorials(request: Request, response: Response){
    ...
    if(request.params.year){
        getTutorialByYear(request.params.year,request,response)
        return
    }
...
export function getTutorialByYear(year: string,request: Request, response: Response){
    Tutorial.getByYear(year,(error, tutorials) => {
        if (error === ErrorTypes.DB_ERROR) {
            response.status(500).send(result.error("Error getting tutorials from database."));
            return;
        }
